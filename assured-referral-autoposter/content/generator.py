...
def _get_system_prompt() -> str:
    """Generate system prompt based on active product config."""
    product = Config.get_product()

    if product:
        brand_name = product.name
        brand_url = product.url
        brand_tagline = product.tagline
        ...
        brand_description = "A platform that helps professionals achieve their goals."
        tone = Config.TONE

    return f"""You are a viral content creator for LinkedIn and Instagram carousels.
You create content for {brand_name} — {brand_description}
...
4. Last slide is ALWAYS a soft CTA that ties the content back to {brand_name} naturally
   - Never be salesy. Frame it as "here's a tool that does this" or "this is why we built {brand_name}"
   - Always include: "{brand_url}" and the tagline
5. Tone: {tone.replace('_', ', ')}. Short sentences. Use line breaks for impact.
...
def generate_content(num_slides: int = 5, brief: dict = None) -> dict:
    ...
    if brief:
        ...
        product = Config.get_product()
        brand_name = product.name if product else Config.BRAND_NAME
...
- Slide {num_slides} = soft {brand_name} CTA with website link
...
6. Last slide is ALWAYS a soft CTA that ties content back to {brand_name} naturally
   - Never be salesy. Frame it as "here's a tool that does this" or "this is why we built {brand_name}"
   - Always include: "{brand_url}" and the tagline
